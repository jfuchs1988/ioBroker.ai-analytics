...
describe('buildTools', () => {
    ...
    it('getHistory resolves the historyInstance from the catalog and delegates to dataAccess', async () => {
        ...
        expect(result).to.deep.equal({ description: undefined, room: undefined, unit: undefined, truncated: false, history: [{ ts: 1, val: 5 }] });
    });
    ...
    it('calculates on-duration and switch count for boolean states', async () => {
        ...
        expect(getHistory.calledOnceWith({}, 'history.0', 'switch.0.x', -316223999000, 1000, 'onchange')).to.equal(true);
    });
