...
const BOOLEAN_LOOKBACK_MS = 10 * 366 * 24 * 3600 * 1000;
    ...
    for (const [room, roomEntries] of rooms) {
        ...
        try {
            ...
            const [windowPoints, heatingPoints] = await Promise.all([
                    getHistory(adapter, windowEntry.historyInstance, windowEntry.sourceId, period.start - BOOLEAN_LOOKBACK_MS, period.end, 'onchange'),
                    getHistory(adapter, heatingEntry.historyInstance, heatingEntry.sourceId, period.start - BOOLEAN_LOOKBACK_MS, period.end, 'onchange'),
                ]);
