    ...
    for (const [room, roomEntries] of rooms) {
        ...
        try {
            ...
            const [windowPoints, heatingPoints] = await Promise.all([
                    getHistory(adapter, windowEntry.historyInstance, windowEntry.sourceId, period.start - 24 * 3600 * 1000, period.end, 'onchange'),
                    getHistory(adapter, heatingEntry.historyInstance, heatingEntry.sourceId, period.start - 24 * 3600 * 1000, period.end, 'onchange'),
            ]);
