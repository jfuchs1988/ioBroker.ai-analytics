...
const replayCaches = new WeakMap();
...
function parseRequest(adapter, id, state) {
    ...
    if (adapter.requireTrustedBridgeSender && !/^system\.adapter\.admin\.\d+$/.test(state.from || '')) {
        if (adapter.log && adapter.log.warn) adapter.log.warn('Admin-Bridge: Anfrage von nicht vertrauenswürdigem Absender ignoriert');
        return null;
    }
    if (Buffer.byteLength(state.val, 'utf8') > MAX_REQUEST_BYTES) {
        if (adapter.log && adapter.log.warn) adapter.log.warn('Admin-Bridge: zu grosse Anfrage ignoriert');
        return null;
    }
    ...
    if (!request || typeof request.id !== 'string' || !request.id.trim() || request.id.length > MAX_REQUEST_ID_LENGTH) {
        if (adapter.log && adapter.log.warn) {
...
async function handleBridgeStateChange(adapter, id, state, dispatch) {
    ...
    const now = Date.now();
    let cache = replayCaches.get(adapter);
    if (!cache) {
        cache = new Map();
        replayCaches.set(adapter, cache);
    }
    for (const [requestId, entry] of cache) {
        if (entry.expiresAt <= now) cache.delete(requestId);
    }

    const fingerprint = state.val;
    const cached = cache.get(request.id);
    let responsePromise;
    if (cached && cached.fingerprint !== fingerprint) {
        responsePromise = Promise.resolve({ id: request.id, ok: false, error: 'Admin-Bridge: Anfrage-ID wurde mit anderem Inhalt wiederverwendet.' });
    } else if (cached) {
        responsePromise = cached.responsePromise;
    } else {
        responsePromise = (async () => {
            try {
                ...
                return { id: request.id, ok: false, error: error && error.message ? error.message : String(error) };
            }
        })();
        if (cache.size >= MAX_REPLAY_ENTRIES) cache.delete(cache.keys().next().value);
        cache.set(request.id, { fingerprint, responsePromise, expiresAt: now + REPLAY_TTL_MS });
    }

    const response = await responsePromise;
    const serializedResponse = JSON.stringify(response);
    if (Buffer.byteLength(serializedResponse, 'utf8') > MAX_CACHED_RESPONSE_BYTES) cache.delete(request.id);
    await adapter.setStateAsync(BRIDGE_STATE, { val: serializedResponse, ack: true });
