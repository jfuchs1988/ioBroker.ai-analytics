...
function classifyFromMetadata(obj) {
    ...
    if (common.type === 'string') {
        return { valueKind: 'text_state', valueKindConfidence: 'low', valueKindSource: 'metadata' };
    }
...
function detectPatternFromSamples(points) {
    ...
    const textPoints = presentPoints.filter((point) => typeof point.val === 'string' && point.val.length <= 1024);
    if (textPoints.length > 0 && textPoints.length !== presentPoints.length) return null;
    if (textPoints.length >= 3) {
        const distinctTexts = new Set(textPoints.map((point) => point.val));
        return distinctTexts.size <= 32 && [...distinctTexts].every((value) => value.length <= 128) ? 'enum_state' : 'text_state';
    }
...
async function classifyValueKind(adapter, obj, historyInstance) {
    ...
    for (const lookbackMs of LOOKBACK_STEPS_MS) {
        ...
        if (signal === 'boolean_state' || signal === 'enum_state' || signal === 'text_state' || signal === 'gauge' || signal === 'daily_reset_counter') {
