...
function validateCatalogEntry(entry, expectedSourceId) {
    ...
    if (entry.reviewReasons !== undefined && (!Array.isArray(entry.reviewReasons) || entry.reviewReasons.length > MAX_REVIEW_REASONS || entry.reviewReasons.some(reason => typeof reason !== 'string' || reason.length > 128 || hasControlCharacter(reason)))) {
        throw new TypeError('Ungueltiges Feld reviewReasons.');
    }
