...
async function listAnthropicModels(config) {
    ...
    const response = await fetchWithTimeout(`${ANTHROPIC_MODELS_URL}?limit=1000`, {
        headers: { 'x-api-key': config.apiKey, 'anthropic-version': ANTHROPIC_VERSION, redirect: 'error' },
        signal: controller.signal,
    }, 15000);
    if (!response.ok) throw await createHttpError(response, 'Anthropic models');
