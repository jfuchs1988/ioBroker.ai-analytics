...
async function listModels(config) {
    const load = async () => {
        switch (config.type) {
            case 'anthropic': return listAnthropicModels(config);
            case 'openai':
            case 'openrouter':
            case 'opencode':
            case 'local': return listOpenAiCompatibleModels(config);
            default: throw new Error(`Unknown provider type: ${config.type}`);
        }
    };
    return withRetry({ chat: load }).chat({ signal: config.signal });
}
