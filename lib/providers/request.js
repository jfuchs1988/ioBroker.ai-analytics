...
async function createHttpError(response, providerName) {
    // Consume only a bounded amount. Expose only a short structured provider message,
    // never the complete provider-controlled body or credentials.
    let detail = '';
    try {
        const text = await readResponseText(response, 8192);
        try {
            const parsed = JSON.parse(text);
            const error = parsed && typeof parsed.error === 'object' ? parsed.error : parsed;
            const parts = [error && error.code, error && error.type, error && error.message].filter(value => typeof value === 'string' && value.trim());
            detail = [...new Set(parts)].join(': ').slice(0, 512);
        } catch (_error) {
            // The status is still useful when the provider did not return JSON.
        }
    } catch (_error) {
        // The HTTP status is sufficient for the public error.
    }
    const error = new Error(`${providerName} API error ${response.status}${detail ? `: ${detail}` : ''}`);
