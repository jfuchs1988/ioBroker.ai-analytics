...
async function getHistory(adapter, historyInstance, sourceId, start, end, aggregate = 'average') {
    if (adapter.log && adapter.log.silly) {
        adapter.log.silly(
            `dataAccess: getHistory an ${historyInstance}: sourceId=${sourceId}, start=${start} (${new Date(start).toISOString()}), end=${end} (${new Date(end).toISOString()}), aggregate=${aggregate}`
        );
    }
