...
async function getHistory(adapter, historyInstance, sourceId, start, end, aggregate = 'average') {
    ...
    const count = isRaw ? DEFAULT_RAW_COUNT + 1 : computeIntervalCount(start, end);
    ...
    const truncated = isRaw && response.result.length >= count;
    if (isRaw && response.result.length > DEFAULT_RAW_COUNT) response.result.length = DEFAULT_RAW_COUNT;
    Object.defineProperty(response.result, 'truncated', { value: truncated, enumerable: false });
    if (truncated && adapter.log && adapter.log.warn) {
        adapter.log.warn(
...
async function compareTimeframes(adapter, historyInstance, sourceId, periodA, periodB, aggregate = 'average', valueKind) {
    ...
    let sumDataA = dataA;
    let sumDataB = dataB;
    if (valueKind === 'gauge' && aggregate === 'average') {
        [sumDataA, sumDataB] = await Promise.all([
            getHistory(adapter, historyInstance, sourceId, periodA.start, periodA.end, 'integral'),
            getHistory(adapter, historyInstance, sourceId, periodB.start, periodB.end, 'integral'),
        ]);
    }
    ...
    return {
        periodA: { start: periodA.start, end: periodA.end, sum: sum(sumDataA), avg: avg(dataA), count: dataA.length },
        periodB: { start: periodB.start, end: periodB.end, sum: sum(sumDataB), avg: avg(dataB), count: dataB.length },
        deltaSum: sum(sumDataB) - sum(sumDataA),
