...
function resolveGroupRoles(groupEntries) {
    ...
    if ([...REQUIRED_ROLES, ...BATTERY_ROLES].some(role => byRole[role] && !COUNTER_VALUE_KINDS.has(byRole[role].valueKind))) return null;
...
async function findEnergyBalanceCandidates(adapter, entries, now = Date.now()) {
    ...
    for (const [groupId, groupEntries] of groups) {
        ...
        try {
            ...
            const dataCompleteness = worstDataCompleteness([...REQUIRED_ROLES, ...BATTERY_ROLES].map((role) => roles[role]).filter(Boolean));
