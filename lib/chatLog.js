...
async function appendChatMessageUnlocked(adapter, role, text) {
...
function appendChatMessage(adapter, role, text) {
    const previous = appendQueues.get(adapter) || Promise.resolve();
    const next = previous.catch(() => {}).then(() => appendChatMessageUnlocked(adapter, role, text));
