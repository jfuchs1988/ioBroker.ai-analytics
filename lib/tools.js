...
function validateInput(name, input, limits = LIMIT_DEFAULTS) {
    ...
    if (name === 'updateCatalogEntry') {
        assertObject(input, name, ['sourceId', 'description', 'category', 'room', 'valueKind']);
        ...
        if (!['description', 'category', 'room', 'valueKind'].some(field => input[field] !== undefined)) throw new Error('Keine Änderung angegeben.');
        return input;
    }
    ...
    if (name === 'updateCatalogEntries') {
        ...
        input.entries.forEach((entry, index) => {
            assertObject(entry, `entries[${index}]`, ['sourceId', 'description', 'category', 'room', 'valueKind']);
    ...
    const definitions = [
        ...
        },
        {
            ...
            description:
                ...
                'Niemals aus einer bloßen Vermutung ändern; die Änderung in der Antwort nennen.',
            inputSchema: {
                type: 'object',
                properties: {
                    sourceId: { type: 'string' },
    ...
    async function execute(name, input) {
        ...
        if (name === 'updateCatalogEntries') {
            ...
            const changes = input.entries.map((change) => {
                ...
                if (!['description', 'category', 'room', 'valueKind'].some((field) => change[field] !== undefined)) {
            ...
            for (const entry of updated) await withCatalogMutation(adapter, () => setCatalogEntry(adapter, entry));
            return { updated };
        }
