...
const VALUE_KINDS = new Set(['gauge', 'boolean_state', 'enum_state', 'text_state', 'daily_reset_counter', 'cumulative_total', 'event_count']);
...
function validateCatalogUpdate(message = {}) {
    ...
    validateStringField(message, 'unit', 64);
...
async function startLicenseActivation(adapter) {
    return adapter.startLicenseActivation();
}

async function getLicenseActivationStatus(adapter) {
    return adapter.getLicenseActivationStatus();
}

async function clearLicenseToken(adapter) {
    return adapter.clearLicenseToken();
}
...
async function updateCatalogEntryAdminUnlocked(
    adapter,
    { sourceId, category, room, description, unit, valueKind, ignored, needsReview, updateFrequency, dataCompleteness, derivedMetricRole, derivedMetricGroupId, derivedMetricInverted, hvacRole } = {},
) {
    validateCatalogUpdate({ sourceId, category, room, description, unit, valueKind, ignored, needsReview, updateFrequency, dataCompleteness, derivedMetricRole, derivedMetricGroupId, derivedMetricInverted, hvacRole });
    ...
    if (unit !== undefined) {
        if (unit === '') delete updated.unit;
        else updated.unit = unit;
    }
    ...
    if (needsReview !== undefined) {
        ...
        if (needsReview === false) delete updated.reviewReasons;
    }
    ...
    if (updateFrequency !== undefined || dataCompleteness !== undefined) {
        updated.dataQualitySource = 'manual';
    }
    ...
    if (valueKind !== undefined && valueKind !== 'boolean_state' && hvacRole === undefined) delete updated.hvacRole;
