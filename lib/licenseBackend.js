...
const DEFAULT_BACKEND_URL = 'https://iob-ai-analytics.jfuchs.de';
...
async function issueEntitlement({ url, activationCode }) {
    ...
    if (typeof result.token !== 'string' || !result.token || typeof result.licenseId !== 'string' || (result.githubLogin !== undefined && typeof result.githubLogin !== 'string') || !Number.isSafeInteger(result.sponsorUntil) || !Number.isSafeInteger(result.tokenExpiresAt)) throw new Error('Lizenzdienst lieferte kein gueltiges Entitlement.');
    return result;
}
...
async function renewEntitlement({ url, token }) {
    ...
    if (typeof result.token !== 'string' || !result.token || typeof result.licenseId !== 'string' || (result.githubLogin !== undefined && typeof result.githubLogin !== 'string') || !Number.isSafeInteger(result.sponsorUntil) || !Number.isSafeInteger(result.tokenExpiresAt)) throw new Error('Lizenzdienst lieferte kein gueltiges Renewal.');
    return result;
}

function shouldRenewEntitlement(tokenExpiresAt, now = Math.floor(Date.now() / 1000)) {
    if (!Number.isSafeInteger(tokenExpiresAt) || tokenExpiresAt <= 0 || !Number.isSafeInteger(now) || now < 0) return false;
    const remaining = tokenExpiresAt - now;
    return remaining > 0 && remaining <= RENEWAL_WINDOW_SECONDS;
}
