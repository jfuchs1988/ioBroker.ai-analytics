...
| `catalog.js` | CRUD auf Katalogeinträgen (Adapter-States), inkl. hartem Löschen; `derivedMetricRole`/`derivedMetricGroupId` verknüpfen optional mehrere Einträge zu einer Energie-Gruppe (nur zusammen gesetzt) — `pv_generation`/`grid_feed_in` für `getSelfConsumption`, zusätzlich `grid_import`/`battery_charge`/`battery_discharge`/`consumption` für die Energiebilanz; `hvacRole` (`window`/`heating`, nur für `boolean_state`) markiert die HVAC-Funktion eines Objekts innerhalb seines `room` | `getCatalogEntry`, `getAllCatalogEntries`, `setCatalogEntry`, `markInactive`, `removeCatalogEntry`, `CATEGORIES`, `DERIVED_METRIC_ROLES`, `HVAC_ROLES` |
...
| `onboarding.js` | Klassifiziert unbekannte Objekte, markiert unsichere als `needsReview`, wendet sichere Defaults für PV/Wärmepumpe, Shelly, Homematic und UniFi an, übernimmt den Raum deterministisch aus `enum.rooms.*`, falls das Objekt dort Mitglied ist; Klassifikations-Batches werden nach Adaptertyp gruppiert. Nach jedem Lauf schlagen rein namensbasierte Heuristiken (kein LLM-Aufruf) ein PV-Erzeugung/Netzeinspeisung-Paar für `getSelfConsumption` sowie pro Raum ein Fenster/Heizung-Paar für `hvacCorrelation.js` vor, wenn genau ein eindeutiger Kandidat je Rolle existiert — sichtbar/änderbar im Geräte-Tab | `runOnboarding(adapter,provider,discoveredObjects) => {classifiedCount,needsReview}`, `buildBatches(objects,batchSize) => object[][]`, `adapterTypeOf(sourceId) => string`, `suggestSelfConsumptionPair(entries)`, `suggestHvacRoles(entries)` |
...
| `hvacCorrelation.js` | Zustands-Overlap-Regel statt Baseline-Vergleich: erkennt pro Raum mit genau einem `hvacRole: 'window'`- und einem `'heating'`-Objekt, ob beide am letzten vollständigen Kalendertag >= 15 Minuten gleichzeitig aktiv waren (Fenster offen bei laufender Heizung) | `findHvacCorrelationCandidates(adapter,entries,now) => {candidates,failedCount}`, `computeOverlapMs(pointsA,pointsB,periodStart,periodEnd)` |
| `energyBalance.js` | Prüft pro `derivedMetricGroupId` mit vollständiger Rollenbelegung (`pv_generation`/`grid_import`/`grid_feed_in`/`consumption` Pflicht, `battery_charge`/`battery_discharge` optional, nur gemeinsam gültig) die Energiebilanz-Residuen der letzten 8 Kalendertage; nutzt `detectDailyAggregateAnomaly` aus `anomalyDetector.js` wieder statt einer eigenen Schwelle | `findEnergyBalanceCandidates(adapter,entries,now) => {candidates,failedCount}` |
...
Das System bleibt bei Ebene 1 (Whitebox der `lib/*`-Module) — bei der aktuellen Größe (22 Module, jeweils fokussiert, siehe [Known Gaps](11-risiken-und-schulden.md) zu Wachstum von `main.js`) liefert eine Ebene-2-Zerlegung (z. B. Whitebox von `providers/`) keinen zusätzlichen Erkenntnisgewinn.
