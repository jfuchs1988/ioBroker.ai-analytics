...
| `tools.js` | Bindet Katalog + Datenzugriff als Werkzeuge; neben Rohdaten und Legacy-Vergleich stehen typbewusste `getPeriodTotal`/`comparePeriods` sowie nutzerbestätigte Batch-Katalogpflege zur Verfügung | `buildTools(adapter) => {definitions, execute}` |
...
| `onboarding.js` | Klassifiziert unbekannte Objekte, markiert unsichere als `needsReview`, wendet sichere Defaults für PV/Wärmepumpe, Shelly, Homematic und UniFi an, übernimmt den Raum deterministisch aus `enum.rooms.*`, falls das Objekt dort Mitglied ist; Klassifikations-Batches werden nach Adaptertyp gruppiert | `runOnboarding(adapter,provider,discoveredObjects) => {classifiedCount,needsReview}`, `buildBatches(objects,batchSize) => object[][]`, `adapterTypeOf(sourceId) => string` |
...
| `anomalyDetector.js` | Statistische Voranalyse numerischer Gauge-Zeitreihen vor der proaktiven LLM-Prüfung; berechnet robuste Abweichungen und filtert unauffällige Objekte heraus | `findAnomalyCandidates(adapter,entries,now)`, `detectSeriesAnomaly`, `median`, `medianAbsoluteDeviation` |
...
Das System bleibt bei Ebene 1 (Whitebox der `lib/*`-Module) — bei der aktuellen Größe (18 Module, jeweils fokussiert, siehe [Known Gaps](11-risiken-und-schulden.md) zu Wachstum von `main.js`) liefert eine Ebene-2-Zerlegung (z. B. Whitebox von `providers/`) keinen zusätzlichen Erkenntnisgewinn.
