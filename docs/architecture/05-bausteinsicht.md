...
| `tools.js` | Bindet Katalog + Datenzugriff als Werkzeuge; neben Rohdaten und Legacy-Vergleich stehen typbewusste `getPeriodTotal`/`comparePeriods` (Periodenberechnung aus `periodValue.js`) sowie nutzerbestätigte Batch-Katalogpflege zur Verfügung. Laufzeitlimits und ein Read-only-Modus schützen autonome Prüfungen | `buildTools(adapter, {readOnly}) => {definitions, execute}` |
