...
│   └── catalogColumns.js    Spalten-Definitionen und Sichtbarkeitsdefaults
...
| `onboarding.js` | Klassifiziert unbekannte Objekte per KI, markiert unsichere als `needsReview`, übernimmt den Raum ausschließlich aus einer expliziten `enum.rooms.*`-Mitgliedschaft oder der KI-Antwort; Klassifikations-Batches werden pro Adapter-Instanz gruppiert (nicht mehr pro Adaptertyp), damit z. B. zwei influxdb-Instanzen getrennt klassifiziert werden. Hersteller-, Adapter- und Namensheuristiken werden nicht verwendet; Analyse-Rollen bleiben bis zur KI- oder Nutzerzuordnung leer. | `runOnboarding(adapter,provider,discoveredObjects) => {classifiedCount,needsReview}`, `buildBatches(objects,batchSize) => object[][]`, `adapterInstanceOf(sourceId) => string` |
