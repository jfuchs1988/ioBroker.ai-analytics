...
- **Admin-Chat-Tab (`admin/tab.js`) nicht gegen eine echte Admin-Instanz verifiziert:** verlässt sich auf ein globales `adapterNamespace`, das kein Standard-ioBroker-Admin-Global ist. Muss beim manuellen Abnahmetest zuerst geprüft werden; falls nicht funktionsfähig, Namespace/Instanz stattdessen aus `window.location.search` ableiten.
