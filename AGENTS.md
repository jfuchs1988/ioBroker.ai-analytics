...
`npm test` muss vor jedem Commit auf `develop` grün sein. Es gibt aktuell keine CI/Linting (bewusst zurückgestellt, siehe Risiken-Doku).
...
Der Adapter bleibt bewusst bei einer Whitebox-Ebene (10 `lib/*`-Module, je 20–100 Zeilen); eine tiefere Zerlegung (z. B. Whitebox von `providers/`) wurde als aktuell nicht lohnend bewertet. Details/Begründungen: [docs/architecture/05-bausteinsicht.md](docs/architecture/05-bausteinsicht.md).
...
- **Branching**: `develop` ist der Arbeits-Branch, alle Änderungen landen hier. `master` wird nur auf ausdrücklichen Nutzerwunsch aktualisiert (kein automatisches Mergen).
